...
// ── image loader ──────────────────────────────────────────────────────────────
const imgDirOld = "C:\\Users\\ADMIN\\.gemini\\antigravity\\brain\\d8893c17-dc77-4210-b0f9-ba6a5c3ee185";
const imgDirNew = "C:\\Users\\ADMIN\\.gemini\\antigravity\\brain\\5eb95e37-236d-4d4a-b8b1-56b43a6dcaea";

// usecase_diagram lấy từ brain mới (không có người que OAuth)
// các ảnh khác vẫn từ brain cũ
const loadImg = (name) => {
  const dir = name === "usecase_diagram" ? imgDirNew : imgDirOld;
  // prefix thực tế trong brain mới là "usecase_no_oauth_actor"
  const prefix = name === "usecase_diagram" ? "usecase_fixed" : name;
  const files = fs.readdirSync(dir).filter(f => f.startsWith(prefix));
  if (!files.length) throw new Error("Image not found: " + prefix + " in " + dir);
  files.sort();
  return fs.readFileSync(path.join(dir, files[files.length - 1]));
};
...
const out = path.join(__dirname, "..", "Chuong3_Goc_Fixed.docx");
